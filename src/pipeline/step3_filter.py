...
def filter_positions(input_file: str, output_file: str) -> Dict[str, Any]:
    """
    Filter position data and calculate distance to liquidation.
    ...
    # Filter positions with liq price and calculate distance
    filtered_rows = []
    skipped_no_liq = 0
    skipped_no_price = 0
    ...
    # Distance breakdown
    close_1pct = sum(1 for r in filtered_rows if r['Distance to Liq (%)'] <= 1)
    close_3pct = sum(1 for r in filtered_rows if r['Distance to Liq (%)'] <= 3)
    close_5pct = sum(1 for r in filtered_rows if r['Distance to Liq (%)'] <= 5)
    close_10pct = sum(1 for r in filtered_rows if r['Distance to Liq (%)'] <= 10)
    stats['within_1pct'] = close_1pct
    stats['within_3pct'] = close_3pct
    stats['within_5pct'] = close_5pct
...
def print_summary(stats: Dict[str, Any], input_file: str, output_file: str):
    ...
    print(f"  Within 1%:  {stats.get('within_1pct', 0):,}")
    print(f"  Within 3%:  {stats['within_3pct']:,}")
    print(f"  Within 5%:  {stats.get('within_5pct', 0):,}")
    print(f"  Within 10%: {stats['within_10pct']:,}")
    if stats.get('by_exchange'):
        print(f"\nBy exchange:")
        for ex, count in sorted(stats['by_exchange'].items()):
            print(f"  {ex}: {count:,}")
    if stats.get('top_tokens'):
        print(f"\nTop tokens ({stats.get('unique_tokens', 0)} total):")
        for tok, count in stats['top_tokens'].items():
            print(f"  {tok}: {count:,}")
