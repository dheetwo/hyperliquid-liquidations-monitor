...
class CascadeResult:
    ...
    def __str__(self) -> str:
        return (
            ...
            f"Est. impact: {self.estimated_price_impact_pct:.2%}" if self.estimated_price_impact_pct else ""
        )
...
class HyperliquidAPIClient:
    ...
    def estimate_price_impact(
        ...
        logger.warning(f"Order book exhausted for {coin} - ${remaining_usd:,.0f} unfilled")
        return None
    ...
    def detect_cascades(
        ...
        if longs:
            cascade = self._build_cascade(coin, longs, current_price, "down")
            if cascade:
                results.append(cascade)

        # Detect cascade for shorts (price moving up)
        if shorts:
            cascade = self._build_cascade(coin, shorts, current_price, "up")
            if cascade:
                results.append(cascade)
    ...
    def _build_cascade(
        ...
        for pos in sorted_positions:
            ...
            if not cascade_positions or reached:
                ...
                impact = self.estimate_price_impact(coin, pos.notional_usd, side, book)
        ...
        total_impact = self.estimate_price_impact(coin, cumulative_notional, total_side, book)
